...
export var WORDTIME_REGEXP = /<(\d+:\d+\.\d+)>/g;
export var LineType;
(function (LineType) {
...
export function parseTime(tags, content) {
    ...
    var cleanContents = content.replace(WORDTIME_REGEXP, '/|/').split("/|/").map(function (f, i, s) { return f.substring(1, i + 1 < s.length ? Math.max(1, f.length - 1) : undefined); });
    var wordTMatches = content.match(WORDTIME_REGEXP);
    if (wordTMatches) {
        cleanContents.shift();
        wordTMatches.forEach(function (wordTag, i) {
            var parsedTags = parseTags(wordTag);
            var _a = __read(parsedTags, 1), tags = _a[0];
            var matches = TIME_REGEXP.exec(tags[0]);
            var minutes = parseFloat(matches[1]);
            var seconds = parseFloat(matches[2].replace(/\s+/g, '').replace(':', '.'));
            wordTimestamps.push({
