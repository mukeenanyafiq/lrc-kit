...
exports.Lrc = exports.timestampToString = exports.padZero = void 0;
...
function timestampToString(timestamp) {
    return "".concat(padZero(Math.floor(timestamp / 60)), ":").concat(padZero((timestamp % 60).toFixed(2)));
}
...
var Lrc = /** @class */ (function () {
    ...
    Lrc.parse = function (text) {
        ...
        text
            .split(/\r\n|[\n\r]/g)
            .map(function (line) {
            return (0, line_parser_1.parseLine)(line);
        })
    ...
    Lrc.prototype.toString = function (opts) {
        ...
        for (var key in this.info) {
            lines.push("[".concat(key, ":").concat(this.info[key], "]"));
        }
        if (opts.combine) {
            // uniqueness
            this.lyrics.forEach(function (lyric) {
            ...
            lyricsList.forEach(function (lyric) {
                lines.push("[".concat(lyric.timestamps
                    .map(function (timestamp) { return timestampToString(timestamp); })
                    .join(']['), "]").concat(lyric.rawContent || ''));
            });
        }
        else {
            this.lyrics.forEach(function (lyric) {
                lines.push("[".concat(timestampToString(lyric.timestamp), "]").concat(lyric.content || ''));
            });
        }
